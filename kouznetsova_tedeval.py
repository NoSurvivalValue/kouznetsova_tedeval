...
def normalize(metric1, metric2, metric3, metric4, metric5):
    ...
    for i in range(len(metric1)):
        norm = round(0.25 * metric1[i] + 0.25 * metric2[i] + 0.25 * metric3[i] + 0.2 * metric4[i] + 0.05 * metric5[i], 4)
...
def average_score(option, gold1, gold2):
    ...
    metric14 = span_difference(option, gold1)
    metric24 = span_difference(option, gold2)
...
def compare_to_generalized_gold():
    ...
    n = int(input('How many other gold standarts will contribute to the genralized gold standard? '))
    for i in range(n):
        gold_path = input('The path to the gold file: ')
    ...
    metric5b = root_accuracy(parsed, golds[-1])
